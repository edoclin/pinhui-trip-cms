    ...
    <advance-query v-if="advancedQuery.show" :conditions="conditions.map" @onSubmit="queryConditions"
      ...
      <template v-slot:selector="scope">
        <el-form-item label="轮播图类型">
          <el-select v-model="scope.formData['selector']['carouselType']" placeholder="请选择轮播图类型" style="width: 100%">
...
const deleteSelected = () => {
  ...
  selectedData.data.forEach(item => ids.push(item.carouselId))

  deleteViewCarouselByIds({ ids }).then(res => {
    ElMessage({
      type: 'success',
      message: res.data
    })
